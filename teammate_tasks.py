...
# groupByKey shuffles (docID, pos) int pairs (~14 bytes each) instead of
# aggregateByKey which shuffles full numpy arrays (~40KB each) — ~14x less shuffle data
allDocsAsNumpyArrays = justDocAndPos.groupByKey().map(
    lambda x: (x[0], buildArray(x[1]))
).persist()

# binary array per doc: 1 if word present, 0 otherwise
zeroOrOne = allDocsAsNumpyArrays.map(lambda x: (x[0], np.where(x[1] > 0, 1, 0)))

# dfArray[i] = number of docs containing word i
